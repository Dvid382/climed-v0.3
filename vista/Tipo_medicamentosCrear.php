...
<div class="container-fluid pt-4 px-4">
     ...
     <?php
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
           ...
           /*  $estatus = ucfirst($_POST['estatus']);
            ...
            $existetipo_medicamentos = $tipo_medicamentoscontroller->verificarTipo_MedicamentosExistente($nombre);

            if ($existetipo_medicamentos) {
                echo "<script> alert ('Error: El Servicio ya existe.')</script>";
            } else {
                $resultado = $tipo_medicamentoscontroller->crear($nombre, $descripcion);

                if ($resultado) {
                    echo "<script> alert '(Error: Estado creado exitosamente.)'</script>";
                     header("Location: Tipo_MedicamentosIndex.php"); 
                } else {
    ...
    <center><h1>Crear Tipo de Medicamento</h1></center>
    <form method="POST">
...
<div class="form-floating mb-3">
    <button class="btn btn-outline-success" type="submit">Crear Tipo de Medicamento. <i class="fa fa-check"></i></button>
