...
<div class="container-fluid pt-4 px-4">
     ...
     <?php
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
           ...
           /*  $estatus = ucfirst($_POST['estatus']);
            ...
            $existeunidadmedidas = $unidadmedidascontroller->verificarUnidadMedidaExistente($nombre);

            if ($existeunidadmedidas) {
                echo "<script> alert ('Error: El Servicio ya existe.')</script>";
            } else {
                $resultado = $unidadmedidascontroller->crear($nombre, $descripcion);

                if ($resultado) {
                    echo "<script> alert '(Error: Estado creado exitosamente.)'</script>";
                     header("Location: UnidadMedidasIndex.php"); 
                } else {
    ...
    <center><h1>Crear forma farmacéutica</h1></center>
    <form method="POST">
...
<div class="form-floating mb-3">
    <button class="btn btn-outline-success" type="submit">Crear forma farmacéutica. <i class="fa fa-check"></i></button>
